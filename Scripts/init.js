// init script I define some libs here and the script should be excuted
// at the init state!



if (fattycurd.vector3df === undefined) {
	
	
	fattycurd.vector3df = function(x, y, z) {
		this.x = x;
		this.y = y;
		this.z = z;
	}
	
	fattycurd.vector3df.prototype.dot = function(v3) {
		return this.x * v3.x + this.y * v3.y + this.z * v3.z;
	}
	
	fattycurd.vector3df.prototype.cross = function(v3) {
		return new fattycurd.vector3df(this.y * v3.z - this.z * v3.y,
									   this.z * v3.x - this.x * v3.z,
									   this.x * v3.y - this.y * v3.x);
	}
	
	fattycurd.vector3df.prototype.length = function() {
		return Math.sqrt(this.x * this.x + this.y * this.y + this.z * this.z);
	}
	
	fattycurd.vector3df.prototype.normalize = function() {
		var invl = 1 / this.length();
		return new fattycurd.vector3df(this.x * invl,
									   this.y * invl,
									   this.z * invl);
	}
	fattycurd.vector3df.prototype.minus = function(v3) {
		return new fattycurd.vector3df(this.x - v3.x,
									   this.y - v3.y,
									   this.z - v3.z);
	}
	fattycurd.vector3df.prototype.mult = function(factor) {
		return new fattycurd.vector3df(this.x * factor,
									   this.y * factor,
									   this.z * factor);
	}
	fattycurd.vector3df.prototype.add = function(v3) {
		return new fattycurd.vector3df(this.x + v3.x,
									   this.y + v3.y,
									   this.z + v3.z);
	}
...
if (fattycurd.EDEBUG === undefined) {
	
	
	fattycurd.EDEBUG = Object.freeze
	({
...
if (fattycurd.EANIM === undefined) {
	
	
	fattycurd.EANIM = Object.freeze
	({
	 EMAT_STAND 				: 0,
	 EMAT_RUN 					: 1,
	 EMAT_ATTACK 				: 2,
...
if (fattycurd.EMOUSE === undefined) {
	
	
	fattycurd.EMOUSE = Object.freeze
	({
...
if (fattycurd.EKEY === undefined) {
	
	
	fattycurd.EKEY = Object.freeze
	({
